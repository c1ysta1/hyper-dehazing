...
class SpectralGrouping(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, num_groups=4):
        super().__init__()
        # 通道数不能被组数整除时（如 182 波段），用零填充补齐输入通道
        self.pad = (num_groups - in_ch % num_groups) % num_groups
        in_ch_padded = in_ch + self.pad
        assert in_ch_padded % num_groups == 0 and out_ch % num_groups == 0
        self.num_groups = num_groups
        self.group_in = in_ch_padded // num_groups
    ...
    def forward(self, x):
        if self.pad:
            x = F.pad(x, (0, 0, 0, 0, 0, self.pad))  # 通道维末尾补零
        B, C, H, W = x.shape
