...
def test_psgnet(args):
    ...
    n_vis = min(5, len(test_ds))
    metrics = {'psnr': [], 'ssim': [], 'sam': [], 'time_ms': []}
    preds = []  # 只保留可视化所需的少量预测，避免内存超限
    with torch.no_grad():
        for i in range(len(test_ds)):
            hazy, clear = test_ds[i]
    ...
    for i in range(n_vis):
        hazy_i, clear_i = test_ds[i]
        axes[i, 0].imshow(to_rgb(hazy_i.numpy()))
        ...
        axes[i, 1].axis('off')
        axes[i, 2].imshow(to_rgb(clear_i.numpy()))
