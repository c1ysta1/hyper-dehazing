...
def plot_psgnet_dehazing():
    ...
    test = torch.load("data/synthetic_haze/test/test_tensor.pth", map_location="cpu")
    hazy_all, clear_all = test["hazy"].float(), test["clear"].float()
...
def plot_spectral_curves():
    ...
    wl = WL_START + np.arange(BANDS) * WL_STEP
    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    for i in range(3):
        # 选植被特征明显的像素（红边位置）
        img = clear_all[i]
        veg = (img[int(BANDS * 0.62)] - img[int(BANDS * 0.35)])  # 近红外-红, (H,W)
