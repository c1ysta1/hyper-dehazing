...
def train(args):
    ...
    H, W = train_hazy.shape[-2:]
    print(f"Train samples: {len(train_hazy)}, Test samples: {len(test_hazy)}")
    print(f"Input channels: {in_ch}, spatial size: {H}x{W}")

    # 数据加载器
    train_ds = TensorDataset(train_hazy, train_clear)
    test_ds = TensorDataset(test_hazy, test_clear)
